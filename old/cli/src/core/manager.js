// From Yarn Let's Dev a Package Manager: https://github.com/yarnpkg/lets-dev-demo/blob/master/index.js
/////////////////////
// --- Chapter 1 ---
...
// For prototype, packages are in ~/coding/wiz/core-packages
const packageRepository = '~/coding/wiz/core-packages/tarballs';
...
async function fetchPackage({
  ...
  if (semver.valid(reference))
    return await fs.readFile(`${packageRepository}/${{name}}-${reference}.tgz`);
...
export async function getPinnedReference({
  name,
  reference
}) {
...
export async function getPackageDependencies({
  ...
  let packageBuffer = await fetchPackage({
    name,
    reference
  });
...
async function getPackageDependencyTree(pace, {
  name,
  reference,
  dependencies
}, available = new Map()) {
  ...
  return {
    ...
    dependencies: await Promise.all(dependencies.filter(volatileDependency => {
      ...
      return await getPackageDependencyTree(pace, Object.assign({}, staticDependency, {
...
async function linkPackages(pace, {
  name,
  reference,
  dependencies
}, cwd) {
  ...
  await Promise.all(dependencies.map(async ({
    name,
    reference,
    dependencies
  }) => {
